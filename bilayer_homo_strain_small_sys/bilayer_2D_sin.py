...
def sinusoidal_strain(c, k_x, k_y, beta=3.37):
    """Produce both the displacement and hopping energy modifier"""
    @pb.site_position_modifier
    def displacement(x, y, z):
        ux = c * np.cos(k_x[0]*x + k_x[1]*y)
        uy = c * np.cos(k_y[0]*x + k_y[1]*y)
        uz = 0
...
period = 1
kx = period * 2 * pi / l2_size * a1 / np.linalg.norm(a1)
ky = period * 2 * pi / l2_size * a2 / np.linalg.norm(a2)
...
strained_model = pb.Model(
    ...
    sinusoidal_strain(0.05, k_x=kx, k_y=ky),
...
bands = pb.solver.arpack(strained_model, k=50).calc_bands(4, -4, step=0.05)
